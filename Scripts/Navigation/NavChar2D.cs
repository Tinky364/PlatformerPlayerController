...
namespace PlatformerPlayerController.Scripts.Navigation
{
    ...
    public class NavChar2D : Area2D
    {
        ...
        public override void _Ready()
        {
            ...
            if (_navAreaPath != "") _navArea = GetNodeOrNull<NavArea2D>(_navAreaPath);
        ...
        public void InterpolateMove(float targetPosX, float duration, Tween.TransitionType transitionType = Tween.TransitionType.Linear, Tween.EaseType easeType = Tween.EaseType.InOut, float delay = 0f)
        {
            ...
            else if (targetPosX > _navArea.ShapeRect.End.x)
                targetPosX = _navArea.ShapeRect.End.x;
        ...
        {
            if (_body.GlobalPosition.x + velocity.x * delta < _navArea.ShapeRect.Position.x && velocity.x < 0
                || _body.GlobalPosition.x + velocity.x * delta > _navArea.ShapeRect.End.x && velocity.x > 0)
            {
                velocity.x = 0;
            }
        ...
        private void FindGroundPositionForArea()
        {
            ...
            _ray = _spaceState.IntersectRay(
                _body.Position + new Vector2(-ShapeExtents.x, 0f),
                _body.Position + new Vector2(-ShapeExtents.x, 300f),
            ...
            if (_ray.Count > 0) // When left ray hits.
            {
                ...
                if (_ray.Count > 0) // When both rays hit.
                {
                    ...
                    if (rightHitPos.DistanceTo(_body.Position) > leftHitPos.DistanceTo(_body.Position))
